function varargout = WRX_Data_GUI2(varargin)
% WRX_Data_GUI2 MATLAB code for WRX_Data_GUI2.fig
%      WRX_Data_GUI2, by itself, creates a new WRX_Data_GUI2 or raises the existing
%      singleton*.
%
%      H = WRX_Data_GUI2 returns the handle to a new WRX_Data_GUI2 or the handle to
%      the existing singleton*.
%
%      WRX_Data_GUI2('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in WRX_Data_GUI2.M with the given input arguments.
%
%      WRX_Data_GUI2('Property','Value',...) creates a new WRX_Data_GUI2 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before WRX_Data_GUI2_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to WRX_Data_GUI2_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help WRX_Data_GUI2

% Last Modified by GUIDE v2.5 07-May-2018 15:42:13

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @WRX_Data_GUI2_OpeningFcn, ...
                   'gui_OutputFcn',  @WRX_Data_GUI2_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before WRX_Data_GUI2 is made visible.
function WRX_Data_GUI2_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to WRX_Data_GUI2 (see VARARGIN)

% Choose default command line output for WRX_Data_GUI2
...
% so window can get raised using WRX_Data_GUI2.
if strcmp(get(hObject,'Visible'),'off')
    plot(rand(5));
end

% UIWAIT makes WRX_Data_GUI2 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = WRX_Data_GUI2_OutputFcn(hObject, eventdata, handles)
...
handles.axes1.XLim = [0,700];
handles.axes2.XLim = [0,700];
...
if (popup_sel_index >= 1) && (popup_sel_index <= 3)
    % the G forces
    y_lab = 'ylabel(''Force in G''''s'',''FontSize'',15)';
...
elseif (popup_sel_index >= 12) && (popup_sel_index <= 19)
    % the temperatures
    y_lab = 'ylabel(''Degrees F'',''FontSize'',15)';
...
elseif (popup_sel_index == 26)||(popup_sel_index == 27)||(popup_sel_index == 118)||...
    ...
    % the voltages
    y_lab = 'ylabel(''Volts'',''FontSize'',15)';
...
elseif (popup_sel_index == 116)||(popup_sel_index == 161)||...
       ...
       (popup_sel_index == 163)
    % absolute (lbs/in^2)
    y_lab = 'ylabel(''PSIA'',''FontSize'',15)';

    % append to the list, say it is a plotted pressure
    handles.plotted(end+1) = 5;

elseif (popup_sel_index == 157)||(popup_sel_index == 158)||(popup_sel_index == 162)
    % Ted wants Torr
    y_lab = 'ylabel(''Torr'',''FontSize'',15)';
    ...
    % counts and such
    y_lab = 'ylabel(''Counts'',''FontSize'',15)';
...
if handles.hold
    if ~all(handles.plotted(1:end-1) == handles.plotted(end))
        ...
        % if I need the logspace
        if (popup_sel_index == 157)||(popup_sel_index == 158)||(popup_sel_index == 162)
...
handles.data.z.GPS_ATM_PRESSURE_time = GPStime + ax;
handles.data.z.GPS_ATM_PRESSURE = handles.alt2press(GPSalt);

handles.names(end+1) = {'GPS_ATM_PRESSURE'};

handles.data.z.GPS_ATM_PRESSURE_Torr_time = GPStime + ax;
handles.data.z.GPS_ATM_PRESSURE_Torr = handles.data.z.GPS_ATM_PRESSURE * 51.7149;

handles.names(end+1) = {'GPS_ATM_PRESSURE_Torr'};

% function for interpolating transition skin pressures in GPS time
trans_in_GPS_time = @(t)interp1(handles.data.z.A138_Transition_Section_Pressure_time,...
                        handles.data.z.A138_Transition_Section_Pressure, t);

% use GPS time
handles.data.z.A138_minus_GPS_time = GPStime + ax;
% use function and subtract at same time
handles.data.z.A138_minus_GPS = (trans_in_GPS_time(...
    handles.data.z.A138_minus_GPS_time)*(150/1023)) - handles.data.z.GPS_ATM_PRESSURE;

% name it!
handles.names(end+1) = {'A138_minus_GPS'};
